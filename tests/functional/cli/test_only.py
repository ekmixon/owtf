...
class OWTFCliOnlyPluginsTest(OWTFCliTestCase):
    ...
    def test_only_one_plugin(self):
        ...
        self.run_owtf(
            "-s", "-o", "OWTF-WVS-001", f"{self.PROTOCOL}://{self.IP}:{self.PORT}"
        )

        # Test WVS-001 active AND external were run.
        self.assert_is_in_logs(
            f"Target: {self.PROTOCOL}://{self.IP}:{self.PORT} -> Plugin: Arachni Unauthenticated (web/active)",
            name="Worker",
            msg="Arachni web active plugin should have been run!",
        )

        self.assert_is_in_logs(
            f"Target: {self.PROTOCOL}://{self.IP}:{self.PORT} -> Plugin: Arachni Unauthenticated (web/external)",
            name="Worker",
