...
plugin_group_re = f'({"|".join(Plugin.get_all_plugin_groups(session))})?'
plugin_type_re = f'({"|".join(Plugin.get_all_plugin_types(session))})?'
...
API_v1_HANDLERS = [
    ...
    tornado.web.url(
        f"/api/v1/plugins/?{plugin_group_re}/?{plugin_type_re}/?{plugin_code_re}/?$",
        PluginDataHandler,
        name="plugins_api_url",
    ),
    tornado.web.url(
        r"/api/v1/plugins/progress/?$",
        ProgressBarHandler,
        name="poutput_count",
    ),
    tornado.web.url(
    ...
    tornado.web.url(
        r"/api/v1/targets/?([0-9]+)?/?$",
        TargetConfigHandler,
        name="targets_api_url",
    ),
    tornado.web.url(
        r"/api/v1/targets/([0-9]+)/urls/?$",
        URLDataHandler,
        name="urls_api_url",
    ),
    tornado.web.url(
    ...
    tornado.web.url(
        f"/api/v1/targets/([0-9]+)/poutput/?{plugin_group_re}/?{plugin_type_re}/?{plugin_code_re}/?$",
        PluginOutputHandler,
    ...
    tornado.web.url(
        r"/api/v1/configuration/?$",
        ConfigurationHandler,
        name="configuration_api_url",
    ),
    tornado.web.url(
        r"/api/v1/dashboard/severitypanel/?$", DashboardPanelHandler
    ),
    tornado.web.url(
    ...
    tornado.web.url(
        r"/api/v1/logout/?$", LogOutHandler, name="logout_api_url"
    ),
    tornado.web.url(
    ...
    tornado.web.url(
        r"/api/v1/generate/otp/?$",
        OtpGenerateHandler,
        name="otp_generate_api_url",
    ),
    tornado.web.url(
